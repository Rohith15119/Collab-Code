...
router.get("/shared/sent", authenticate, async (req, res) => {
  try {
    const sessions = await Session.find({
      ownerId: req.user.id,
...
router.post("/:roomId/share", authenticate, shareLimiter, async (req, res) => {
...
router.delete("/:roomId/share", authenticate, async (req, res) => {
  const { roomId } = req.params;
  const { email } = req.body;
  ...
  try {
    ...
    // 3. Remove from sharedWith atomically
...
router.get("/:roomId/collaborators", authenticate, async (req, res) => {
  const { roomId } = req.params;
